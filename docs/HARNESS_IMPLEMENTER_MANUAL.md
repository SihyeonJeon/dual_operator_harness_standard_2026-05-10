...
13. Ensure visualization backend descriptors exist under `harness/viz/`, with
    ...
    evidence exist.
14. Ensure H0, H1, and F0 planning runway tasks exist but production work
    is not started.
15. Write `harness/IMPLEMENTER_HANDOFF.md`.
16. Write and update `harness/IMPLEMENTER_HOOKS.md` and
    `harness/IMPLEMENTER_HOOKS_RUN.json`.
17. Write project-root `guide_for_human.md`.
18. Write project-root bilingual `README.md`.
19. Write `harness/runtime/OFFLINE_OPERATION.md`.
20. Write `harness/runtime/REMOTE_OPERATION_POLICY.md`.
21. Write `harness/runtime/CLOUD_VIZ_OPERATOR_GUIDE.md`.
22. Write `harness/evals/README.md`, `harness/evals/golden_suite.json`, and
    `schemas/eval-suite.schema.json`.
23. Ensure the target project has its own validator, `scripts/harnessctl.py`,
    `scripts/implementer_hooks.py`, and schemas or equivalent local validation
    command.
24. Run `scripts/validate_harness.py <target-project>`.
25. Write `harness/SCAFFOLDING_REPORT.md`.
