...
- [ ] Root `scripts/harnessctl.py` exists and exposes `validate`, `event`, `report`, `viz-spec-check`, `eval-run`, `viz-export`, `archive`, `context-pack`, `worker-brief`, `model-route`, `task-packet`, `current-research`, `cross-feedback`, `concept-check`, `software-feedback`, `preregister-benchmark`, `blind-redact`, `council-decision`, and `recovery-evidence`.
